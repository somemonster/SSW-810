...
class Repository:
    ...
    def open_majors_file(self,path,split_type):
        ...
        for major,tflag,tcourse in file_reader(path,number_files,'\t'):
            self.majors[tcourse] = Majors(major,tflag,tcourse)
    ...
    def print_majors(self):
        pt = PrettyTable(field_names=['major','tflag','tcourse'])
        for tcourse in self.majors:
            pt.add_row(self.majors[tcourse].Majors_info())
        print(pt)
...
class Majors:
    def __init__(self,major,tflag,tcourse):
        self.major=major
        self.tflag= tflag
        self.tcourse=tcourse

    def Majors_info(self):
        return [self.major,self.tflag,self.tcourse]
...
def main():
    directory = r'/Users/123/python/lec10'
